...
float random(uint n) {
	n = (n << 13U) ^ n;
	n = n * (n * n * 15731U + 0x789221U) + 0x17631289U;
	return float(n & uint(0x7fffffffU)) / float(0x7fffffff);
}


const vec3 grass_color = vec3(0.0, 0.7, 0.0);
const float max_grass_height = 0.25; // change it in cpp file as well.. im too lazy, to send it as uniform
const float grass_width = 1;
const float density = 512;

void main () {
   vec3 output_grass_color = grass_color;

    uvec2 tposu = uvec2(vs_texcoord * density);
    uint seed = tposu.x + 123u * tposu.y + 123u;
    float noise = random(seed);

#if 1
    float threshold = world_position.y / max_grass_height;
    if (noise <= threshold) {
        discard;
    }    

#else
    // cylindrical grass doesnt look as good as i thought

    float threshold = world_position.y / max_grass_height;
    vec2 uv_per_grass = fract(vs_texcoord * density);
    vec2 centered_uv_per_grass = uv_per_grass * 2.0 - 1.0;
    float distance_from_center = length(centered_uv_per_grass);

    if (distance_from_center > noise - threshold) {
        discard;
    }   

#endif
    output_grass_color = grass_color * threshold;

    // LIGHTNING
    // vec3 normal = normalize(normal_matrix * vs_normal);
    // vec3 view_direction = normalize(camera_position - world_position);
    // output_grass_color *= all_lights(normal, view_direction, world_position);
    
    frag_color = vec4(output_grass_color, 1.0);
